  ...
  methods: {
    ...
    handleSearch() {
      if (this.searchVal !== null) {
        ...
        this.getCountryByName(this.searchVal)
          ...
          .catch(() => {
            this.handleSearchByKeyword(this.searchVal);
          });
      }
    },
    search(keyword) {
    ...
    handleSearchByKeyword(keyword) {
      ...
      this.allCountries.map((item) => {
        similarityCount = this.similarity(keyword, item.name)
        if (similarityCount > maxCount) {
          maxCount = similarityCount;
          foundedCountry = item;
        }
      })
      foundedCountry !== {} ? this.setData([foundedCountry]) : this.resetData();
    },
    similarity(s1, s2) {
      var longer = s1;
      var shorter = s2;
